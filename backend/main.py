...
def _interview_question_dict(question: models.InterviewQuestion, include_answer: bool = True) -> Dict[str, Any]:
    payload = {
        ...
        "tags": [tag.strip() for tag in (question.tags or "").split(",") if tag.strip()],
...
def _get_owned_question(db: Session, question_id: int, current_user: models.User) -> models.InterviewQuestion:
    question = db.query(models.InterviewQuestion).filter(
        models.InterviewQuestion.id == question_id,
        models.InterviewQuestion.is_active == 1,
    ).first()
    if question is None:
...
def get_interview_categories(current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    rows = db.query(
        models.InterviewQuestion.category,
        func.count(models.InterviewQuestion.id),
    ).filter(models.InterviewQuestion.is_active == 1).group_by(models.InterviewQuestion.category).all()
...
def get_interview_questions(
    ...
    query = db.query(models.InterviewQuestion).filter(models.InterviewQuestion.is_active == 1)
    if category:
    ...
    if search.strip():
        ...
        query = query.filter(or_(
            models.InterviewQuestion.question.ilike(term),
            models.InterviewQuestion.answer.ilike(term),
            models.InterviewQuestion.category.ilike(term),
            models.InterviewQuestion.subcategory.ilike(term),
            models.InterviewQuestion.tags.ilike(term),
        ))
    total = query.count()
...
def get_interview_question(question_id: int, current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    return _interview_question_dict(_get_owned_question(db, question_id, current_user))


@app.post("/api/interview/questions/{question_id}/bookmark")
def bookmark_interview_question(question_id: int, current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    _get_owned_question(db, question_id, current_user)
    existing = db.query(models.InterviewBookmark).filter(
        models.InterviewBookmark.user_id == current_user.id,
        models.InterviewBookmark.question_id == question_id,
    ).first()
    if existing is None:
        db.add(models.InterviewBookmark(user_id=current_user.id, question_id=question_id))
        db.commit()
    return {"saved": True}


@app.delete("/api/interview/questions/{question_id}/bookmark")
def remove_interview_bookmark(question_id: int, current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    bookmark = db.query(models.InterviewBookmark).filter(
        models.InterviewBookmark.user_id == current_user.id,
        models.InterviewBookmark.question_id == question_id,
    ).first()
    if bookmark:
...
@app.get("/api/interview/bookmarks")
def get_interview_bookmarks(page: int = 1, page_size: int = 20, current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    query = db.query(models.InterviewQuestion).join(
        models.InterviewBookmark, models.InterviewBookmark.question_id == models.InterviewQuestion.id
    ).filter(
        models.InterviewBookmark.user_id == current_user.id,
        models.InterviewQuestion.is_active == 1,
    ).order_by(models.InterviewBookmark.created_at.desc())
    total = query.count()
    items = query.offset((max(1, page) - 1) * min(max(1, page_size), 50)).limit(min(max(1, page_size), 50)).all()
    return {"items": [_interview_question_dict(item, include_answer=False) for item in items], "total": total}


@app.post("/api/interview/questions/{question_id}/practice")
def record_interview_practice(question_id: int, payload: schemas.InterviewPracticeCreate, current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    _get_owned_question(db, question_id, current_user)
    practice = models.InterviewPractice(
        user_id=current_user.id,
        question_id=question_id,
        answer_submitted=(payload.answer_submitted or "").strip()[:10000],
        completed=1 if payload.completed else 0,
    )
    db.add(practice)
    db.commit()
    return {"message": "Practice recorded", "practice_id": practice.id}


@app.post("/api/interview/chat")
def interview_chat(payload: Dict[str, Any], current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    prompt = str(payload.get("message") or "").strip()
    if not prompt:
        raise HTTPException(status_code=422, detail="Message is required")
    category = str(payload.get("category") or "").strip()
    category_aliases = {"hr": "HR / Behavioral Round", "technical": "Technical Fundamentals", "sql": "SQL / Database Round", "coding": "Programming / Coding Round", "projects": "Project Discussion Round", "resume": "Resume-Based Interview", "managerial": "Managerial Round", "gd": "Group Discussion", "aptitude": "Online Assessment / Aptitude"}
    category = category_aliases.get(category.lower(), category)
    query = db.query(models.InterviewQuestion).filter(models.InterviewQuestion.is_active == 1)
    if category:
        query = query.filter(models.InterviewQuestion.category == category)
    terms = [term for term in re.findall(r"[A-Za-z0-9]+", prompt.lower()) if len(term) > 2][:8]
    if terms:
        query = query.filter(or_(*[models.InterviewQuestion.question.ilike(f"%{term}%") for term in terms]))
    question = query.order_by(models.InterviewQuestion.id.asc()).first()
    if question is None and category:
        question = db.query(models.InterviewQuestion).filter(models.InterviewQuestion.category == category, models.InterviewQuestion.is_active == 1).first()
    if question is None:
        return {"answer": "Choose a category or ask about a topic such as SQL joins, deadlocks, coding arrays, projects, resume questions, or HR behavior. I use the curated interview bank and will not present invented company-specific questions as guaranteed.", "question": None, "matches": []}
    return {"answer": question.answer, "question": _interview_question_dict(question), "matches": [_interview_question_dict(item, include_answer=False) for item in query.limit(5).all()]}


@app.get("/api/admin/interview/questions")
def get_admin_interview_questions(search: str = "", category: str = "", difficulty: str = "", active: Optional[int] = None, order: str = "desc", page: int = 1, page_size: int = 20, current_user: models.User = Depends(require_admin_user), db: Session = Depends(get_db)):
    query = db.query(models.InterviewQuestion)
    if search.strip():
        term = f"%{search.strip()}%"
        query = query.filter(or_(models.InterviewQuestion.question.ilike(term), models.InterviewQuestion.tags.ilike(term), models.InterviewQuestion.category.ilike(term)))
    if category:
        query = query.filter(models.InterviewQuestion.category == category)
    if difficulty:
        query = query.filter(models.InterviewQuestion.difficulty == difficulty)
    if active is not None:
        query = query.filter(models.InterviewQuestion.is_active == active)
    ordering = models.InterviewQuestion.category.asc() if order == "category" else models.InterviewQuestion.difficulty.asc() if order == "difficulty" else models.InterviewQuestion.created_at.asc() if order == "oldest" else models.InterviewQuestion.created_at.desc()
    total = query.count()
    size = min(max(1, page_size), 100)
    items = query.order_by(ordering).offset((max(1, page) - 1) * size).limit(size).all()
    return {"items": [_interview_question_dict(item) for item in items], "total": total, "page": max(1, page), "page_size": size, "total_pages": max(1, (total + size - 1) // size)}


@app.get("/api/admin/interview/stats")
def get_admin_interview_stats(current_user: models.User = Depends(require_admin_user), db: Session = Depends(get_db)):
    questions = db.query(models.InterviewQuestion)
    return {"total": questions.count(), "active": questions.filter(models.InterviewQuestion.is_active == 1).count(), "inactive": questions.filter(models.InterviewQuestion.is_active == 0).count(), "categories": questions.with_entities(models.InterviewQuestion.category).distinct().count(), "beginner": questions.filter(models.InterviewQuestion.difficulty == "Beginner").count(), "intermediate": questions.filter(models.InterviewQuestion.difficulty == "Intermediate").count(), "advanced": questions.filter(models.InterviewQuestion.difficulty == "Advanced").count()}


@app.post("/api/admin/interview/questions")
def create_admin_interview_question(payload: schemas.InterviewQuestionCreate, current_user: models.User = Depends(require_admin_user), db: Session = Depends(get_db)):
    ...
    item = models.InterviewQuestion(**values)
    db.add(item)
    db.commit()
    db.refresh(item)
    return _interview_question_dict(item)


@app.put("/api/admin/interview/questions/{question_id}")
def update_admin_interview_question(question_id: int, payload: schemas.InterviewQuestionCreate, current_user: models.User = Depends(require_admin_user), db: Session = Depends(get_db)):
    item = db.query(models.InterviewQuestion).filter(models.InterviewQuestion.id == question_id).first()
    if item is None:
        raise HTTPException(status_code=404, detail="Interview question not found")
    values = payload.model_dump()
    if values["difficulty"] not in {"Beginner", "Intermediate", "Advanced"}:
    ...
    for field, value in values.items():
        setattr(item, field, 1 if field == "is_active" and value else 0 if field == "is_active" else value)
    db.commit()
    db.refresh(item)
    return _interview_question_dict(item)


@app.patch("/api/admin/interview/questions/{question_id}/status")
def update_admin_interview_question_status(question_id: int, payload: Dict[str, Any], current_user: models.User = Depends(require_admin_user), db: Session = Depends(get_db)):
    item = db.query(models.InterviewQuestion).filter(models.InterviewQuestion.id == question_id).first()
    if item is None:
        raise HTTPException(status_code=404, detail="Interview question not found")
    item.is_active = 1 if bool(payload.get("is_active")) else 0
    db.commit()
    return {"id": item.id, "is_active": bool(item.is_active)}


@app.delete("/api/admin/interview/questions/{question_id}")
def deactivate_admin_interview_question(question_id: int, current_user: models.User = Depends(require_admin_user), db: Session = Depends(get_db)):
    item = db.query(models.InterviewQuestion).filter(models.InterviewQuestion.id == question_id).first()
    if item is None:
        raise HTTPException(status_code=404, detail="Interview question not found")
    item.is_active = 0
    db.commit()
    return {"message": "Interview question deactivated", "id": item.id}
