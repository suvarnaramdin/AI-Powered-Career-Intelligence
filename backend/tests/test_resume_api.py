...
class ResumeAPITests(unittest.TestCase):
    ...
    def test_upload_and_list_resume(self):
        ...
        list_response = self.client.get("/resumes/jane@example.com")
        ...
        download_response = self.client.get(f"/resume/{resume_id}/download")
        self.assertEqual(download_response.status_code, 200)
        self.assertEqual(download_response.headers["content-disposition"].startswith("attachment"), True)

        view_response = self.client.get(f"/resume/{resume_id}/view")
        ...
        job_response = self.client.post("/job-description", json=job_description_payload)
        self.assertEqual(job_response.status_code, 200)
        job_id = job_response.json()["id"]

        analysis_response = self.client.post(f"/ats/analyze/{resume_id}/{job_id}")
        ...
        delete_response = self.client.delete(f"/resume/{resume_id}")
        self.assertEqual(delete_response.status_code, 200)

        missing_response = self.client.get(f"/resume/{resume_id}/download")
