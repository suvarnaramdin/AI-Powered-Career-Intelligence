...
export default function AdminNotificationsPage() {
  ...
  const openNotification = async (item) => {
    ...
    if (item.type === "feedback") navigate("/admin/feedback");
    if (item.type === "resume") navigate("/admin/resumes");
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3"><button type="button" onClick={markAllRead} disabled={!data.unread_count} className="text-sm font-semibold text-amber-700 disabled:opacity-40">Mark all read</button><span className="rounded-full bg-amber-100 px-2.5 py-1 text-xs font-semibold text-amber-700">{data.unread_count} unread</span></div>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {data.items.length === 0 ? <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-6 text-center text-slate-500">No notifications available.</div> : data.items.map((item) => (
              <div key={item.id} role="button" tabIndex={0} onClick={() => openNotification(item)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") openNotification(item); }} className={`w-full cursor-pointer rounded-2xl border p-4 text-left ${item.read ? "border-slate-200 bg-slate-50" : "border-amber-200 bg-amber-50"}`}>
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  <div>
                    <p className="text-base font-semibold text-slate-900">{item.title}</p>
                    <p className="text-sm text-slate-600">{item.description}</p>
                  </div>
                  {!item.read ? <button type="button" onClick={(event) => { event.stopPropagation(); markRead(item.id); }} className="rounded-xl bg-amber-500 px-3 py-2 text-sm font-semibold text-white hover:bg-amber-400">Mark read</button> : <span className="rounded-full bg-slate-200 px-2.5 py-1 text-xs font-semibold text-slate-700">Read</span>}
                </div>
